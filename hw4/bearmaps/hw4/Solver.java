...
public class Solver<Vertex> implements ShortestPathsSolver<Vertex> {
    ...
    public Solver(AStarGraph<Vertex> G, Vertex start, Vertex goal, double timeout) {
            ...
            for (WeightedEdge<Vertex> e : G.neighbors(p)) {
                ...
                if (!distTo.containsKey(q) || distTo.get(p) + w < distTo.get(q)) {
                    ...
                    if (pq.contains(q)) {
                        ...
                        //System.out.println(distTo.get(q) + G.estimatedDistanceToGoal(q, goal));
                    } else {
